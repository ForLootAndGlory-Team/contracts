...
library MathHelper {
    ...
    function calculBonusPercent(
        uint256 amount,
        int32 bonus
    ) internal pure returns (uint256) {
        uint256 percent = 100;
        // Calculer le montant du bonus, qui peut être positif ou négatif
        int256 bonusAmount = (int256(amount) * int256(bonus)) / int256(percent);
        // Calculer le nouveau montant en ajoutant le bonusAmount à amount
        // bonusAmount peut être négatif, ce qui réduirait effectivement le montant
        int256 newAmount = int256(amount) + bonusAmount;
