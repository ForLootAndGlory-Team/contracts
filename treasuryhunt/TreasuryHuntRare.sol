...
import "./InternalHunt.sol";
import {MathHelper} from "../libraries/MathHelper.sol";

contract TreasuryHuntRare is InternalHunt {
    ...
    {
        difficulty = 3270;
        COIN = LibTheTreasureSea.TheTreasureSeaEnum.COIN_SILVER;
    ...
    function launchTreasuryHunt(
        ...
        uint256 characterLevel = MathHelper.sqrt(
            uint256(characterInfos.experience)
        );

        if (characterInfos.thirst < thristPoint)
            revert LibErrors.BepPointTooLow();
        if (characterLevel > maxLevelToHunt || characterLevel < minLevelToHunt)
            revert LibErrors.CharacterLevelNotAllowed();
    ...
    function batchLaunchTreasuryHunt(
        ...
        if (len > 20) revert LibErrors.MaxHuntReach();
        uint256[] memory randomWords = getRandomNumbers(
            ((uint256(LibTreasureHunt.numWords) * len) * numberOfHunt)
        );
        uint256 index;
        for (uint256 i; i < len; i++) {
            ...
            if (
                characterLevel > maxLevelToHunt ||
                characterLevel < minLevelToHunt
            ) revert LibErrors.CharacterLevelNotAllowed();
            ...
            uint256[] memory _randomWords = new uint256[](
                LibTreasureHunt.numWords * numberOfHunt
            );
            for (
                uint256 j;
                j < (LibTreasureHunt.numWords * numberOfHunt);
                j++
            ) {
    ...
    /**
     * @dev Internal function to launch a treasury hunt.
    ...
    function _launchTreasuryHunt(
        ...
        huntings.AmountHunt = numberOfHunt;

        characters.addTreasuryHuntResult(characterId, thristPoint, 0);
        _ClaimTreasuryHunt(huntings, _randomWords);
    }
}
