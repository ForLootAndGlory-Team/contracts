    ...
    NFTOrders
{
    ...
    ///        `proxyMultiERC1155OrderCallback` on `msg.sender` after
    ...
    function buyERC1155(
        ...
        _buyERC1155(
            sellOrder,
            signature,
            BuyParams(erc1155BuyAmount, callbackData)
        );
    }
    ...
    function batchCancelERC1155Orders(
        ...
        if (revertIfIncomplete) {
            for (uint256 i = 0; i < sellOrders.length; i++) {
                ...
                _buyERC1155(
                    sellOrders[i],
                    signatures[i],
                    BuyParams(erc1155FillAmounts[i], callbackData[i])
                );
                successes[i] = true;
            }
        } else {
    ...
    function onERC1155Received(
        ...
        (
            ...
            LibSignature.Signature memory signature
                ...
                data,
                (LibNFTOrder.ERC1155Order, LibSignature.Signature)
            );
        ...
        if (msg.sender != address(buyOrder.erc1155Token)) {
            revert LibNFTOrdersRichErrors.ERC1155TokenMismatch(
                msg.sender,
