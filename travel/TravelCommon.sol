...
contract TravelCommon is InternalTravel {
    function initialize(
        address _theSea,
        ...
        __Pausable_init();
        rarity = LibTheTreasureSea.TheTreasureSeaRarity.COMMON; // COMMON
        chest = LibTheTreasureSea.TheTreasureSeaEnum.CHEST_COMMON;
        StartRhumFill = 5000;
    ...
    /**
     * @dev Launches a travel for the specified ship.
     ...
     * @notice Emits a `TravelLaunched` event upon successful launch of the travel.
     * @notice Throws a `NeedCaptain` error if the ship does not have a captain.
     * @notice Throws a `DurabilityTooLow` error if the ship's durability is too low.
     * @notice Throws a `StuffTooHigh` error if the ship does not meet the requirements for traveling.
     * @notice Throws a `YouAreNotOwnerOfShip` error if the caller is not the owner of the ship.
     */
    ...
    function attack(
        ...
        if (shipsinfos[attackerId].owner != _DeleguateMsgSender())
            revert LibErrors.YouAreNotOwnerOfShip();
        if (shipsinfos[attackerId].owner == shipsinfos[defenderId].owner)
            revert LibErrors.ThisTargetIsYourShip();
    ...
    /**
     * @dev Checks if a traveling ship can be shipped based on the rarity of its components.
     * @param shipId The ID of the ship to check.
     * @return result A boolean indicating whether the ship can be shipped or not.
     */
    function canShipTraveling(
        uint256 shipId
